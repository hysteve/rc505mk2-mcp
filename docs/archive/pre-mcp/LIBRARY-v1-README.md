...
```json
{
  "mcpServers": {
    "rc505mk2": {
      "command": "node",
      "args": ["/absolute/path/to/packages/rc505mk2-lib/dist/mcp/server.js"]
    }
  }
}
```
...
```bash
claude mcp add rc505mk2 node /absolute/path/to/packages/rc505mk2-lib/dist/mcp/server.js
```
...
| `lookup_fx_params` | Get detailed parameter definitions for a specific FX (tag mappings, value ranges, enum options). |
| `list_fx_modules` | Browse the FX module preset library. Filter by category, effect, or usage pattern. |
| `get_fx_module` | Get full details of a specific FX module by ID, including params and pairing suggestions. |
| `create_fx_module` | Create a new FX module preset and write it to the module library. |
| `build_rack_config` | Build a MemoryConfig JSON from FX chain descriptions with human-readable values. |
| `create_preset` | One-step preset creation: builds config and writes RC0 files to disk. Can build from scratch or from a curated rack preset ID. |
| `generate_memory` | Generate RC0 file pair (A + B) from a MemoryConfig and write to disk. |
| `parse_memory` | Parse RC0 XML back into a MemoryConfig JSON object. |
| `list_rack_presets` | Browse the curated rack preset library. Filter by genre or FX type. |
...
| `upload_memory` | Upload RC0 files to a connected device with automatic backup. Supports merge mode to preserve existing banks. |
